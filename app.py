...
def format_time_indian(value, format_str='%I:%M %p'):
    """Endpoint/Function to Handle Format time indian."""
    if value is None:
        return ""
    try:
...
def get_db():
    """Endpoint/Function to Handle Get db."""
    if 'db' not in g:
...
def close_db(exception):
    """Endpoint/Function to Handle Close db."""
    db = g.pop('db', None)
    if db is not None: db.close()

def init_db():
    """Endpoint/Function to Handle Init db."""
...
def initdb_command():
    """Endpoint/Function to Handle Initdb command."""
...
class User(UserMixin):
    ...
    def __init__(self, id, email, full_name):
        """Endpoint/Function to Handle   init  ."""
        self.id, self.email, self.full_name = id, email, full_name

@login_manager.user_loader
def load_user(user_id):
    """Endpoint/Function to Handle Load user."""
...
def allowed_file(filename):
    """Endpoint/Function to Handle Allowed file."""
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in app.config['ALLOWED_EXTENSIONS']

def log_activity(activity_type, description):
    """Endpoint/Function to Handle Log activity."""
...
def calculate_and_save_adherence(profile_id):
    """Endpoint/Function to Handle Calculate and save adherence."""
    """
...
def login():
    """Endpoint/Function to Handle Login."""
...
@login_required
def logout():
    """Endpoint/Function to Handle Logout."""
...
@login_required
def profiles():
    """Endpoint/Function to Handle Profiles."""
    return render_template('profiles.html')

@app.route('/switch_profile/<int:profile_id>')
@login_required
def switch_profile(profile_id):
    """Endpoint/Function to Handle Switch profile."""
    db = get_db()
...
@login_required
def medicines():
    """Endpoint/Function to Handle Medicines."""
    profile_id = session.get('active_profile_id')
...
@login_required
def add_medicine():
    """Endpoint/Function to Handle Add medicine."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    form = MedicineForm()
    if form.validate_on_submit():
...
@login_required
def reminders():
    """Endpoint/Function to Handle Reminders."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    form = ReminderForm()
    db = get_db()
...
@login_required
def appointments():
    """Endpoint/Function to Handle Appointments."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
...
@login_required
def stats():
    """Endpoint/Function to Handle Stats."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
@login_required
def emergency():
    """Endpoint/Function to Handle Emergency."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
...
@login_required
def history():
    """Endpoint/Function to Handle History."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    form = HistoryForm()
    db = get_db()
...
def register():
    """Endpoint/Function to Handle Register."""
    if current_user.is_authenticated: return redirect(url_for('dashboard'))
...
@login_required
def add_profile():
    """Endpoint/Function to Handle Add profile."""
    form = ProfileForm()
    if form.validate_on_submit():
        db = get_db()
        dob_obj = None
...
@login_required
def profile():
    """Endpoint/Function to Handle Profile."""
...
def manage_profile(profile_id):
    """Endpoint/Function to Handle Manage profile."""
    db = get_db()
...
def uploaded_file(profile_id, filename):
    """Endpoint/Function to Handle Uploaded file."""
    ...
    user_upload_folder = os.path.join(app.root_path, app.config['UPLOAD_FOLDER'], str(profile_id))
    if not os.path.exists(os.path.join(user_upload_folder, filename)):
        return send_from_directory(os.path.join(app.root_path, app.config['UPLOAD_FOLDER']), 'default.png')
    return send_from_directory(user_upload_folder, filename)

@app.route('/dashboard')
@login_required
def dashboard():
    """Endpoint/Function to Handle Dashboard."""
    profile_id = session.get('active_profile_id')
    if not profile_id:
        return redirect(url_for('profiles'))
...
def take_medicine(med_id):
    """Endpoint/Function to Handle Take medicine."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
def add_stock(med_id):
    """Endpoint/Function to Handle Add stock."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
    med = db.execute('SELECT name, current_stock FROM medicines WHERE id = ? AND profile_id = ?', (med_id, profile_id)).fetchone()
...
def delete_medicine(med_id):
    """Endpoint/Function to Handle Delete medicine."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
def delete_reminder(rem_id):
    """Endpoint/Function to Handle Delete reminder."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
def delete_history(rec_id):
    """Endpoint/Function to Handle Delete history."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
def delete_appointment(apt_id):
    """Endpoint/Function to Handle Delete appointment."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
def delete_emergency(cid):
    """Endpoint/Function to Handle Delete emergency."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
@login_required
def generate_qr():
    """Endpoint/Function to Handle Generate qr."""
    profile_id = session.get('active_profile_id')
    if not profile_id: return redirect(url_for('profiles'))
    db = get_db()
...
def serve_shared_file(token, filename):
    """Endpoint/Function to Handle Serve shared file."""
    db = get_db()
...
@login_required
def delete_account():
    """Endpoint/Function to Handle Delete account."""
    db = get_db()
...
@login_required
def export_data():
    """Endpoint/Function to Handle Export data."""
...
def delete_profile(profile_id):
    """Endpoint/Function to Handle Delete profile."""
    db = get_db()
...
@login_required
def my_account():
    """Endpoint/Function to Handle My account."""
...
@login_required
def intake_log():
    """Endpoint/Function to Handle Intake log."""
    profile_id = session.get('active_profile_id')
    if not profile_id:
...
def delete_intake_log(log_id):
    """Endpoint/Function to Handle Delete intake log."""
    profile_id = session.get('active_profile_id')
    if not profile_id:
        return redirect(url_for('profiles'))
...
def manager_dashboard():
    """Endpoint/Function to Handle Manager dashboard."""
    db = get_db()
    today_name = datetime.now().strftime('%a')
